...
| --- | --- | --- | --- | --- |
| [x] | Environment prepared |  | 2026-06-07 | Automated environment setup tests added and passed. |
...
| --- | --- | --- | --- |
| [x] | Install dependencies inside the project virtual environment. | Application dependencies install without errors. |  |
| [x] | Start the application from `venv` using `python main.py`. | Application starts without Python path errors. |  |
| [x] | Verify `.env` database settings. | Host, port, database, and user point to the intended test database. |  |
| [x] | Verify the application does not use production data during testing. | Test database is clearly separated from production. |  |
| [x] | Verify logs are written and readable. | Startup, login, database, and application errors appear in logs. |  |
| [x] | Verify screen resolution support: small laptop, standard desktop, wide screen. | UI remains usable without clipped text or overlapping controls. |  |

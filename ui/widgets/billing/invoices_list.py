...
class InvoicesListWidget(QWidget):
    ...
    def load_data(self):
        ...
        if hasattr(self.manager, 'external_transfers'):
            ...
            for row, t in enumerate(transfers):
                ...
                self.table.setItem(row, 0, QTableWidgetItem(str(t['Transfer_ID'])))
                self.table.setItem(row, 1, QTableWidgetItem(str(t.get('Transaction_Date', '')).split('.')[0]))
                self.table.setItem(row, 2, QTableWidgetItem(t.get('Partner_Name', '-')))
                
                amt_item = QTableWidgetItem(f"{float(t.get('Total_Amount', 0)):,.2f}")
    ...
    def export_transfer_to_pdf(self, transfer_id):
        """
        توليد ملف PDF احترافي مع معالجة ذكية للمسارات في جذر المشروع.
        """
        ...
        # 1. تحديد مسار جذر المشروع بدقة (Project Root)
        # بما أنك تشغل البرنامج من main.py، فإن cwd هو المجلد الرئيسي
        cwd = os.getcwd()
        settings_path = os.path.join(cwd, "config.json")

        # تتبع مسار ملف الإعدادات
        print(f"[*] Current Working Directory: {cwd}")
        print(f"[*] Target Settings Path: {settings_path}")

        if not os.path.exists(settings_path):
            # محاولة أخيرة: الصعود للأعلى من موقع الملف الحالي
            base_dir = os.path.dirname(os.path.abspath(__file__))
            settings_path = os.path.abspath(os.path.join(base_dir, "..", "..", "..", "pdf_settings.json"))
            print(f"[*] Fallback Path (Up 3 levels): {settings_path}")

        # 2. تحميل الإعدادات من JSON
        try:
            if not os.path.exists(settings_path):
        ...
        # 3. التحقق من مسار الشعار (Logo)
        logo_path = settings.get('banner_path', '')
        print(f"[*] Logo path from JSON: {logo_path}")
        
        if not os.path.exists(logo_path):
            # محاولة تصحيح مسار الشعار إذا كان نسبياً
            potential_logo = os.path.join(cwd, "ui", "logo2.png")
            if os.path.exists(potential_logo):
                logo_path = potential_logo
                print(f"[!] Logo not found in JSON path, using: {logo_path}")
            else:
                print(f"[!] WARNING: Logo file still not found!")

        # 4. جلب البيانات من مدير البيانات (Manager)
        try:
            ...
            print("[+] Database records retrieved successfully.")
        except Exception as e:
            print(f"[!] DB ERROR: {e}")
            return
        ...
        default_name = f"{settings.get('doc_title', 'BL')}_{partner_clean}_{transfer_id}.pdf"
        path, _ = QFileDialog.getSaveFileName(self, "Enregistrer PDF", default_name, "PDF Files (*.pdf)")
        if not path: return

        # 6. بناء المستند (ReportLab)
        try:
            ...
            def draw_header_compact(canvas, doc):
                ...
                # إضافة إزاحة بسيطة للأسفل (مثلاً 0.2 سم) لتجنب حواف الصفحة
                y_offset = settings.get('banner_img_y_cm', 0.2) * cm
                img_y = PAGE_HEIGHT - img_h - y_offset
                
                if os.path.exists(logo_path):
                    try:
                        # حذف mask='auto' لحل مشكلة الشفافية
                        canvas.drawImage(logo_path, img_x, img_y, width=img_w, height=img_h)
                        print(f"DEBUG: Image drawn at Y={img_y}")
                    except Exception as e:
                        print(f"DEBUG ERROR: Failed to draw image: {e}")
                else:
                    canvas.setStrokeColor(colors.red)
                    canvas.rect(img_x, img_y, img_w, img_h, stroke=1)
                
                canvas.restoreState()
            # --- محتوى المعلومات (Bank & Partner) ---
            current_time = datetime.now().strftime('%d/%m/%Y %H:%M')
            left_text = (
                f"<font size=14 color='{settings.get('theme_color')}'><b>{settings.get('doc_title')} N°: {transfer_id}</b></font><br/><br/>"
            ...
            for item in details_data:
                ...
                obs = f"Valeur: {line_val:,.2f} DA" if is_billable else "<font color='red'>Gratuit/Don</font>"
            ...
            print(f"[+] PDF successfully created: {path}")
            ...
            QMessageBox.critical(self, "Erreur", f"Build failed: {str(e)}")

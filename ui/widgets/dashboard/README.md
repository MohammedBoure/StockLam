...
- **`charts_section.py`** : Section graphique professionnelle (`ChartsSection`) affichant l'analyse comparative des flux de stock sous forme de colonnes (Entrées/Achats en Vert 🟩, Sorties/Consommation en Rouge 🟥) avec filtres historiques locaux (7j, 30j, 3m, 6m, 1an, personnalisé), granularités temporelles (Jour, Semaine, Mois), métriques KPI récapitulatives et infobulles riches.
...
- **`statistics_tabs.py`** : Onglets d'analyse statistique avancée, incluant la valorisation détaillée du stock (`StockValuationTab`) et l'analyse des pertes (`WasteAnalysisTab`).

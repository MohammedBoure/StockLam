...
_ALL_PERMISSIONS = [
    ...
    "nav_finance", 
    "nav_services", 
    "nav_history",     # واجهة التتبع المستقلة
    "nav_settings",
    "nav_market",

    # --- تبويبات لوحة القيادة (Dashboard) ---
    "tab_dash_overview", 
    "tab_dash_reception", 
...
class SchemaInitializerMixin:
    ...
    def _create_default_admin(self, cursor):
        """إنشاء حساب المسؤول الافتراضي أو تحديث صلاحياته لتشمل المفاتيح الجديدة"""
        ...
        perms_json = json.dumps(_ALL_PERMISSIONS)
        ...
        if not admin_exists:
            ...
            query = """
                ...
                VALUES (%s, %s, %s, %s, %s)
            """
            cursor.execute(query, ('admin', hashed_pw, 'Admin', 'Administrateur Système', perms_json))
        else:
            # تحديث الصلاحيات للأدمن الموجود لضمان ظهور الواجهات الجديدة بعد التعديل
